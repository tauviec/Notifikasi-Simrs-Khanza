...
3. Double-klik `start.vbs` (bukan `start.bat` — `.bat` selalu memunculkan jendela terminal sekilas tiap kali diklik, `.vbs` menjalankan perintah yang sama tanpa jendela sama sekali). Ikon lonceng hijau akan muncul di system tray (pojok kanan bawah, mungkin perlu diklik panah "^" untuk melihat ikon yang disembunyikan Windows). `start.bat` tetap ada sebagai cadangan kalau `start.vbs` bermasalah di suatu PC.
